...
def heavy(request: HttpRequest) -> HttpResponse:
    ...
    images = _collect_heavy_images(settings.BASE_DIR / "AM", source="am")
    image_source = "AM"
...
def heavy_asset(request: HttpRequest, asset_path: str) -> FileResponse:
    ...
    root = (settings.BASE_DIR / "AM").resolve()
    ...
    try:
        ...
        if settings.AUTH_MODE == "keycloak":
            ...
            messages.info(
                request,
                _(
                    "Votre compte Animation Messe doit être synchronisé depuis cARThographie."
                ),
            )
            store_pending_provision_state(
...
def _keycloak_diagnostic_causes(diagnostic: dict[str, object]) -> list[str]:
    ...
    if (
        ...
        return [
            _("Secret client Keycloak invalide ou non monté dans le conteneur AM."),
            _("Client ID différent entre AM et Keycloak."),
    ...
    if stage:
        return [
            _("Consultez les logs AM autour de keycloak_http_error."),
...
def keycloak_diagnostic(request: HttpRequest) -> HttpResponse:
    ...
    checks = [
        _(
            "Vérifier que KEYCLOAK_CLIENT_SECRET_FILE pointe vers un fichier monté dans le conteneur AM."
        ),
        _("Vérifier KEYCLOAK_CLIENT_ID et KEYCLOAK_REDIRECT_URI dans .env.prod."),
        _("Chercher keycloak_http_error dans les logs AM."),
