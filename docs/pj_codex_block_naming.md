# Nomenclature Codex pour nommer les blocs de page

Ce document définit la nomenclature Animation Messe à utiliser dans les prompts adressés à Codex pour désigner les blocs visibles de la structure de page.

Il s'applique aux templates Django qui heritent du socle `templates/base.html` et utilisent les blocs de page communs (`section_title`, `page_tools`, `content`, etc.).
...
Noms de référence à utiliser dans les échanges avec Codex :
