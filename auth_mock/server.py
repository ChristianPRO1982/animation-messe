...
class AuthMockHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/":
            self._write_text(
                HTTPStatus.OK,
                "AM auth_mock is running. Open /login?return_to=http://localhost:8000/auth/callback/ or start from AM.",
            )
            return
...
<head>
  ...
  <title>AM auth mock</title>
  <style>
    body {{ font-family: Arial, sans-serif; background: #f4f0ea; color: #1f2933; margin: 0; padding: 3rem 1.5rem; }}
    main {{ max-width: 42rem; margin: 0 auto; background: #fffaf4; border: 1px solid #d8c7b5; border-radius: 12px; padding: 2rem; box-shadow: 0 10px 30px rgba(48, 36, 24, 0.08); }}
    h1 {{ margin-top: 0; }}
    ul {{ line-height: 1.8; }}
...
<body>
  <main>
    <h1>AM auth mock</h1>
    <p>Sélectionnez un utilisateur de test à renvoyer vers Animation Messe.</p>
    <ul>
      {"".join(links)}
    </ul>
  </main>
